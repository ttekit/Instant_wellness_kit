...
function preloadImage(src, fallbackSeed) {
    const img = new Image();
...
function ProductGrid() {
    ...
    useEffect(() => {
        ...
        cardRefs.current.forEach(card => { if (card)
            observer.observe(card); });
        return () => observer.disconnect();
    }, []);
    const handlePreload = (p) => {
        if (!preloaded.current.has(p.id)) {
            preloadImage(p.image, p.id);
            preloaded.current.add(p.id);
        }
    };
    const handleSelectKit = (p) => {
        if (!isLoggedIn()) {
    ...
    return (<>
      <div className="bg-[#f3f6f4] px-8 py-12 flex flex-col items-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 w-full max-w-4xl">
          {products.map((p, i) => (<div key={p.id} ref={el => { cardRefs.current[i] = el; }} className="reveal bg-white rounded-xl overflow-hidden shadow-sm border border-gray-100 flex flex-col select-none" style={{ transitionDelay: `${i * 100}ms` }} onMouseEnter={() => handlePreload(p)} onFocus={() => handlePreload(p)}>
              <div className="relative">
                <img src={p.image} alt={p.name} className="w-full h-36 object-cover bg-gray-100" onError={e => { e.target.src = `https://picsum.photos/seed/${p.id}/500/300`; }}/>
                {p.popular && (<span className="absolute top-2 left-2 bg-yellow-400 text-white text-[10px] font-bold px-2.5 py-0.5 rounded-full">Popular</span>)}
              </div>
              ...
              <div className="p-4 flex flex-col flex-1">
                ...
                <button onClick={() => handleSelectKit(p)} className="card-btn w-full bg-[#2596be] hover:bg-[#1a7a9e] text-white text-xs font-semibold py-2 rounded-lg">
                  Select This Kit
                </button>
              </div>
            </div>))}
...
//# sourceMappingURL=ProductGrid.jsx.map