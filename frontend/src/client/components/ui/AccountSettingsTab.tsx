    ...
    const loadProfile = async () => {
      try {
        ...
        const res = await fetch(
          `${import.meta.env.VITE_API_USERS_URL}/${currentUserId}`,
          {
            headers: {
    ...
    try {
      ...
      if (res.ok) {
        ...
        window.dispatchEvent(
          new CustomEvent("profile_updated", { detail: { name, email } }),
        );
        setSaved(true);
    ...
    <div className="tab-enter space-y-4 max-w-lg">
      <div className="bg-white rounded-2xl border border-gray-100 p-6">
        ...
        <div className="field-row">
          <button
            onClick={handleSave}
            className="sbtn bg-[#1a5c3a] hover:bg-[#154d30] text-white text-xs font-semibold px-5 py-2.5 rounded-xl transition-all"
          >
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6 flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-1.5 text-xs font-semibold text-red-500 border border-red-200 hover:bg-red-50 px-3 py-2 rounded-xl transition-colors"
        >
          <svg
      ...
      {showLogin && <LoginModal onClose={() => navigate("/")} blurBg />}
    </div>
  );
}
