...
export default function AccountTabs() {
    ...
    return (<div>
      <div className="flex bg-[#eef2ee] rounded-full p-1 mb-8 max-w-sm">
        {tabs.map(t => (<button key={t.id} onClick={() => setActive(t.id)} className={`flex-1 flex items-center justify-center gap-1.5 text-xs py-2 rounded-full font-medium transition-all outline-none border-0 ${active === t.id ? 'bg-white shadow text-gray-800' : 'bg-transparent text-gray-500 hover:text-gray-700'}`}>
            {t.icon}{t.label}
          </button>))}
      </div>
...
//# sourceMappingURL=AccountTabs.jsx.map