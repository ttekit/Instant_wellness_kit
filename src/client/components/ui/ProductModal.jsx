...
export default function ProductModal({ product, onClose }) {
    ...
    const handleLocation = (c) => { setCoords(c); setStep(3); };
    const handleConfirm = (id) => { setServerOrderId(`INK-${id}`); setStep(4); };
    return (<div className={`fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 ${closing ? 'b-out' : 'b-in'}`}>
      <div className={`bg-white rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl ${closing ? 'm-out' : 'm-in'}`}>

        <div className="relative h-32">
          <img src={product.image} alt={product.name} className="w-full h-full object-cover" onError={e => { e.target.src = `https://picsum.photos/seed/${product.id}/500/300`; }}/>
...
//# sourceMappingURL=ProductModal.jsx.map