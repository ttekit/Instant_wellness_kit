...
export default function StepLocation({ onNext }) {
    ...
    const detect = () => {
        ...
        navigator.geolocation.getCurrentPosition(({ coords: { latitude: lat, longitude: lng } }) => {
            setLoading(false);
            if (!isInNYC(lat, lng))
                return setError('Sorry, delivery is only available in New York City.');
            setCoords({ lat, lng });
        }, () => { setLoading(false); setError('Could not detect location. Please allow location access.'); }, { timeout: 10000 });
    };
    const submitManual = () => {
        const lat = parseFloat(manualLat), lng = parseFloat(manualLng);
        if (isNaN(lat) || isNaN(lng))
            return setError('Please enter valid coordinates.');
        if (!isInNYC(lat, lng))
            return setError('Sorry, delivery is only available in New York City.');
        setError('');
        setCoords({ lat, lng });
    ...
    return (<div className="field-row">
      ...
      {coords ? (<div className="bg-[#e8f5e9] rounded-xl p-4 mb-4 text-center">
        ...
        </div>) : manualMode ? (<div className="mb-4 space-y-3">
          ...
          {[
            ...
            ].map(({ label, value, set, placeholder }) => (<div key={label} className="iw flex items-center border !border-gray-200 rounded-xl px-3 py-2 !bg-white">
              <span className="text-gray-400 text-xs mr-2 w-6">{label}</span>
              <input type="number" step="any" placeholder={placeholder} value={value} onChange={e => set(e.target.value)} className="flex-1 text-sm outline-none bg-transparent text-gray-800 select-text"/>
            </div>))}
        </div>) : (<div className="bg-gray-50 rounded-xl p-5 mb-4 text-center">
      ...
      {!coords && !manualMode && (<>
          <button onClick={detect} disabled={loading} className="sbtn w-full bg-[#2596be] text-white py-2.5 rounded-xl text-sm font-semibold outline-none border-0 mb-2 flex items-center justify-center gap-2 disabled:opacity-60">
          ...
          <button onClick={() => { setManualMode(true); setError(''); }} className="w-full text-xs text-gray-400 hover:text-gray-600 bg-transparent outline-none border-0 py-1 transition-colors">
            Enter coordinates manually
          </button>
        </>)}

      {!coords && manualMode && (<div className="space-y-2">
          ...
          <button onClick={() => { setManualMode(false); setError(''); }} className="w-full text-xs text-gray-400 hover:text-gray-600 bg-transparent outline-none border-0 py-1 transition-colors">
            ← Use GPS instead
          </button>
        </div>)}

      {coords && (<button onClick={() => onNext(coords)} className="sbtn w-full bg-[#1a5c3a] text-white py-3 rounded-xl text-sm font-semibold outline-none border-0">
          Continue to Order
        </button>)}
    </div>);
}
