...
import { Order, Jurisdiction } from "../types/Orders.types";
...
export default function Orders() {
  ...
  const handleAddJurisdiction = () => {
    ...
    const newJurisdiction = jurisdictions.find(
      (j) => j.id === selectedJurisdictionToAdd,
    );

    if (
      newJurisdiction &&
      !editOrder.jurisdictions.some(
        (oj) => oj.jurisdiction.id === newJurisdiction.id,
      )
    ) {
      setEditOrder({
        ...editOrder,
      ...
      setSelectedJurisdictionToAdd(""); // Reset selection
    }
  };

  const handleRemoveJurisdiction = (jurisdictionId: number) => {
    if (!editOrder) return;

    setEditOrder({
      ...editOrder,
      jurisdictions: editOrder.jurisdictions.filter(
        (oj) => oj.jurisdiction.id !== jurisdictionId,
      ),
    });
  };
  ...
  const handleStatusChange = async (orderId: number, newStatus: string) => {
    try {
      const response = await fetch(
        `http://localhost:4200/orders/${orderId}/status`,
        {
          method: "PATCH",
          headers: {
  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen">
      ...
      {!loading && !error && (
        <>
          <SearchBar
            placeholder="Search by order ID, or jurisdiction..."
            value={search}
            onChange={setSearch}
          />
          <div className="mt-4">
              ...
              <div className="flex flex-col gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    value={editOrder.customerName || "N/A"}
                    readOnly
                  />
                </div>
                <div>
                  ...
                  <input
                    id="editKit"
                    className="mt-1 w-full border border-gray-200 rounded-lg px-3 py-2 text-sm"
                    value={editOrder.orderPackage?.package || "N/A"}
                    readOnly
                  />
                </div>
                <div>
                    ...
                    editOrder.jurisdictions.length > 0 ? (
                      <ul className="list-none space-y-1">
                        {editOrder.jurisdictions.map((orderJurisdiction) => (
                          <li
                            key={orderJurisdiction.jurisdiction.id}
                            ...
                            <button
                              ...
                              onClick={() =>
                                handleRemoveJurisdiction(
                                  orderJurisdiction.jurisdiction.id,
                                )
                              }
                ...
                <div>
                  ...
                  <div className="mt-1 flex gap-2">
                    <select
                      ...
                      value={selectedJurisdictionToAdd}
                      onChange={(e) =>
                        setSelectedJurisdictionToAdd(Number(e.target.value))
                      }
                    ...
                    <button
                      ...
                      disabled={!selectedJurisdictionToAdd}
