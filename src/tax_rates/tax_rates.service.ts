...
export class TaxRatesService {
    ...
    async create(createTaxRateDto: CreateTaxRateDto) {
        ...
        if (!jurisditcion) {
            throw new BadRequestException(`Jurisdiction  with ID ${createTaxRateDto.jurisdiction_id}`)
        }
    ...
    async update(id: number, updateTaxRateDto: UpdateTaxRateDto) {
        const existingTaxRate = await this.prisma.taxRate.findUnique({ where: { id } })
        if (!existingTaxRate) {
            throw new NotFoundException(`Tax rate with ${id} not found`)
        }
        ...
        return this.prisma.taxRate.update({
            ...
            data: {
                ...dataToUpdate,
                ...(jurisdiction_id && { jurisdiction: { connect: { id: jurisdiction_id } } }),
